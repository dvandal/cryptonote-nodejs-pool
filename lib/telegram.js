...
exports.sendMessage = function(messageText, chatId) {
    ...
    // Check if group id has been specified
    if (!config.telegram.groupId || config.telegram.groupId === '') {
        log('error', logSystem, 'No telegram group id specified in configuration!');
        return ;
    }
    var groupId = config.telegram.groupId.replace(/@/, '');

    // Set telegram API URL
    var action = "sendMessage";
    var params = { chat_id: '@' + groupId, text: '*' + messageText + '*', parse_mode: 'Markdown' };
    ...
    https.get(apiURL, function(request) {
	    ...
	    log('info', logSystem, 'Telegram message sent to @%s: %s', [groupId, messageText]);
        });
