...
function getExchangeMarkets(exchange, callback) {
    ...
    // TradeOgre
        ...
        apiInterfaces.jsonHttpRequest('app.stex.com', 443, '', function(error, response) {
            ...
            if (!error && response) {
                for (var model in response) {
		            var pieces = response[model]['market_name'].split('_');
                    var target = pieces[1];
                    var symbol = pieces[0];
    ...
    // Btc-Alpha
        ...
        apiInterfaces.jsonHttpRequest('btc-alpha.com', 443, '', function(error, response) {
            ...
            if (!error && response) {
                for (var model in response) {
                    var pieces = response[model]['pair'].split('_');
                    var target = pieces[1];
                    var symbol = pieces[0];

                    var price = +response[model]['price'];
                    if (price === 0) continue;
...
function getExchangePrice(exchange, base, target, callback) {
    ...
    // Btc-Alpha
        ...
        apiInterfaces.jsonHttpRequest('btc-alpha.com', 443, '', function(error, response) {
            ...
            let price = response[0] != undefined ? response[0]['price'] : null;
            if (!price) log('warn', logSystem, 'No exchange data for %s using %s', [ticker, exchange]);
