        ...
        function(blocks, callback){
            ...
            if (config.blockUnlocker.donations.enabled) {
                ...
                redisClient.multi(getDonationCommands).exec(function(error, replies) {
                    ...
                    removedDonations = true;
                });

                if (!removedDonations) {
                    log('error', logSystem, 'An error occured while processing worker donations; aborting block unlock pass');
                    callback(true);
                    return;
                }
            }
            ...
            for (var worker in payments) {
                ...
                if (worker in donate && donate[worker] > 0)
                    unlockedBlockCommands.push(['hincrby', config.coin + ':workers:' + worker, 'donations', donate[worker]]);
            }
