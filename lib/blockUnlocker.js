    ...
    async.waterfall([
        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                var effort = Math.round(block.shares / block.difficulty * 100);
                redisClient.hgetall(config.coin + ':notifications', function(error, data) {
                    ...
                    for (var address in data) {
                        ...
                        emailSystem.sendEmail(
                            ...
                            {
                                'HEIGHT': block.height,
                                'TIME': dateFormat(new Date(block.time), 'yyyy-mm-dd HH:MM:ss Z'),
				'HASH': block.hash,
				'REWARD': getReadableCoins(block.reward, 4, false),
