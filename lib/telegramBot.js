...
else if (config.telegram.remote) {
    log('error', logSystem, 'Telegram is configured in remote relay mode');
}
...
function sendMinerStats(telegramMsg, address) {
    ...
    apiInterfaces.pool('/stats_address?address='+address, function(error, data) {
        ...
        if (config.blockUnlocker.donations.enabled) {
            var lvl = data.stats.donation_level;
            response += 'Donation level: ' + lvl + '% ' + utils.getDonationSmiley(lvl) + '\n';
        }
        response += 'Balance: ' + minerBalance + '\n';
        response += 'Paid: ' + minerPaid + '\n';
        if (config.blockUnlocker.donations.enabled) {
            response += 'Donations: ' + utils.getReadableCoins(data.stats.donations);
        }
        if (data.workers && data.workers.length > 0) {
            ...
            for (var i in data.workers) {
                ...
                response += workerName + ': ' + workerHashrate + '; ' + (
                    utils.getReadableHashRate(data.workers[i].hashrate_1h) + ' (1h); ' +
                    utils.getReadableHashRate(data.workers[i].hashrate_24h) + ' (24h)') + '\n';
                f = false;
            }
        }
