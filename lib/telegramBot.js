...
if (periodicalStats && statsInterval > 0 && channel) {
    ...
    setInterval(function(){ sendPoolStats('@'+channel); }, statsInterval * 1000);
}
...
function sendPoolStats(chatId) {
    apiInterfaces.pool('/stats', function(error, stats) {    
        ...
        var lastBlock = (stats.pool.lastBlockFound) ? notifications.formatDate(new Date(parseInt(stats.pool.lastBlockFound))) : 'Never';
        var currentEffort = stats.pool.roundHashes ? (stats.pool.roundHashes / stats.network.difficulty * 100).toFixed(1) + '%' : 'N/A';

        var response = '';
        response += 'Pool Hashrate: ' + poolHashrate + '\n';
        response += 'Network Hashrate: ' +  networkHashrate + '\n';
        response += 'Network Difficulty: ' + networkDiff + '\n';
        response += 'Blockchain Height: ' + networkHeight + '\n';
        response += 'Connected Miners: ' + poolMiners + '\n';
        response += 'Active Workers: ' + poolWorkers + '\n';
        response += 'Blocks Found: ' + poolBlocks + '\n';
        response += 'Last block: ' + lastBlock + '\n';
