...
exports.sendEmail =  function(email, subject, content) {
    ...
    // If we are a remote node, stash the message to be relayed by the primary node.
    if (config.email.remote) {
        log('info', logSystem, 'E-mail to %s (%s) stored for relaying by master node');
        redisClient.rpush(config.coin + ':relay:email', JSON.stringify({ 'email': email, 'subject': subject, 'content': content }));
        return;
    }
