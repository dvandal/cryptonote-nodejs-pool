...
exports.sendEmail = function(email, subject, template, variables) {
    ...
    if (config.email.disabledTemplates && config.email.disabledTemplates.length > 0) {
        for (var i = 0; i <= config.email.disabledTemplates.length; i++) {
            if (config.email.disabledTemplates[i] === template) {
