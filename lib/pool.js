...
var active_connections_key = config.coin + ':active_connections';
var ports_key = config.coin + ':ports';
if (config.multipool) {
    active_connections_key += ':' + config.multipool;
    ports_key += ':' + config.multipool;
}
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            var customDonation;
            if (config.poolServer.customDonationAmount && config.poolServer.customDonationAmount.enabled) {
                var escaped_delimiter = ((config.poolServer.customDonationAmount.addressDelimiter || '%') + '').replace(
                    /([.\\+*?\[\]^$()])/g, '\\$1');
                login = login.replace(new RegExp(escaped_delimiter + "(\\d+(?:\\.\\d+)?|\\.\\d+)" + escaped_delimiter),
                    function(match, p1) {
                        ...
                        customDonation -= config.blockUnlocker.poolFee;
                        if (customDonation < 0) customDonation = 0.0;
                        return '';
                    });
            }
            ...
            newConnectedWorker(miner, customDonation);
            break;
        case 'getjob':
            if (!miner){
 ...
 **/
function newConnectedWorker(miner, donation){
    ...
    if (config.blockUnlocker.donations.enabled) {
        if (donation === undefined) {
            for (let p of config.poolServer.ports) {
                if (p.port == miner.port && 'donation' in p) {
                    donation = p.donation;
                }
            }
    ...
    redisClient.hincrby(ports_key + ':'+miner.port, 'users', 1);
    redisClient.hset(config.coin + ':workers:' + miner.login, 'donation_level', donation);

    redisClient.hincrby(active_connections_key, miner.login + '~' + miner.workerName, 1, function(error, connectedWorkers) {
        if (connectedWorkers === 1) {
            notifications.sendToMiner(miner.login, 'workerConnected', {
                ...
                'PORT': miner.port,
                'DONATION': donation + '%',
                'DONATION_SMILEY': utils.getDonationSmiley(donation),
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ...
        ['persist', config.coin + ':workers:' + miner.login], //, (86400 * cleanupInterval)],
        ['persist', config.coin + ':payments:' + miner.login] //, (86400 * cleanupInterval)]
    ];
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate){
            ...
            notifications.sendToAll('blockFound', {
                ...
                'SHARES': totalShares,
                'EFFORT': Math.round(totalShares / blockTemplate.difficulty * 100) + '%',
                'MINER': miner.login.substring(0,7)+'...'+miner.login.substring(miner.login.length-7)
            });
        }
...
function startPoolServerTcp(callback){
    log('info', logSystem, 'Clear values for connected ' + (config.multipool ? 'local ' + config.multipool + ' pool ' : '')
            + 'workers in redis database.');
    redisClient.del(active_connections_key);
        ...
        if (portData.ssl) {
            if (!config.poolServer.sslCert) {
                ...
                tls.createServer(options, socketResponder).listen(portData.port, function (error, result) {
                    ...
                    redisClient.del(ports_key + ':'+portData.port);
                    redisClient.hset(ports_key + ':'+portData.port, 'port', portData.port);
