...
BlockTemplate.prototype = {
    ...
    nextBlob: function(){
        ...
        if (config.poolServer.mergedMining) {
            if (config.childCoinPoW && config.childCoinPoW > 0) {
                return utils.cnUtil.convert_blob(this.buffer, cnBlobType, currentChildBlockTemplate.buffer, config.childCoinPoW).toString('hex');
            }
            return utils.cnUtil.convert_blob(this.buffer, cnBlobType, currentChildBlockTemplate.buffer).toString('hex');
        }
        if (config.childCoinPoW && config.childCoinPoW > 0) {
            return utils.cnUtil.convert_blob(this.buffer, cnBlobType, 0, config.childCoinPoW).toString('hex');
        }
...
function processShare(miner, job, blockTemplate, childBlockTemplate, params){
    ...
    if (config.poolServer.mergedMining) {
        if (config.childCoinPoW && config.childCoinPoW > 0) {
            shareBuffer = utils.cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), cnBlobType, childBlockTemplate.buffer, config.childCoinPoW);
        } else {
            shareBuffer = utils.cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), cnBlobType, childBlockTemplate.buffer);
        }
    } else {
        if (config.childCoinPoW && config.childCoinPoW > 0) {
            shareBuffer = utils.cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), cnBlobType, 0, config.childCoinPoW);
        } else {
    ...
    if (config.poolServer.mergedMining){
        if (hashDiff.ge(childBlockTemplate.difficulty)){
            ...
            apiInterfaces.rpcDaemon('submitblock', [mergedBuffer.toString('hex')], function(error, result){
                if (error){
                    log('error', logSystem, 'Error submitting  child block at height %d from %s@%s, share type: "%s" - %j', [job.height, miner.login, miner.ip, shareType, error]);
