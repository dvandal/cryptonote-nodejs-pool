...
const cryptonight_v3 = require('multi-hashing')['cryptonight_variant'];
const cryptonight_v4 = require('cryptonight-hashing')['cryptonight'];
...
function newConnectedWorker(miner){
    ...
    if (miner.difficulty){
        if(miner.noRetarget){
            ...
            log('info', logSystem, 'Miner difficulty set to %s', [miner.difficulty]);
        }
    }
...
function processShare(miner, job, blockTemplate, nonce, resultHash,sendReply){
   ...
   const convertedBlob = cnUtil.convert_blob(shareBuffer);
	    var hash;
	    var shareType;
	    ...
	    if (hash.toString('hex') !== resultHash) {
		        //    if(shareType === 'valid'){
			//	                    hash = cryptonight_v3(convertedBlob,cn_variant);
			//	                    if (hash.toString('hex') !== resultHash) {
...
function startPoolServerTcp(callback){
    async.each(config.poolServer.ports, function(portData, cback){
        var handleMessage = function(socket, jsonData, pushMessage){
            ...
            var miner = handleMinerMethod(jsonData.method, jsonData.params, socket.remoteAddress, portData, sendReply, pushMessage);
            /*if(miner){
                socket.on('close',function(){
                    removeConnectedWorker(miner,"Miner disconnected");
                    log('info', logSystem, 'Miner / disconnected %s@%s on port', [miner.login, miner.ip, miner.port]);
