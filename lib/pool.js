...
console.log("convert_blob merged block");
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, isChildCoin){
    ...
    if (blockCandidate){
        ...
        redisCommands.push(['rename', coin + ':scores:roundCurrent', coin + ':scores:round' + job.height]);
        redisCommands.push(['rename', coin + ':shares_actual:roundCurrent', coin + ':shares_actual:round' + job.height]);
        redisCommands.push(['hgetall', coin + ':scores:round' + job.height]);
        redisCommands.push(['hgetall', coin + ':shares_actual:round' + job.height]);
    }
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (blockCandidate){
            ...
            redisClient.zadd(coin + ':blocks:candidates', job.height, [
                hashHex,
                Date.now() / 1000 | 0,
