...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    if (miner.workerName) {
        redisCommands.push(['hincrby', config.coin + ':shares:roundCurrent', miner.login + '+' + miner.workerName, job.score]);
	redisCommands.push(['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty, miner.login + '+' + miner.workerName, dateNow].join(':')]);
	redisCommands.push(['hincrby', config.coin + ':workers:' + miner.login + '+' + miner.workerName, 'hashes', job.difficulty]);
        redisCommands.push(['hset', config.coin + ':workers:' + miner.login + '+' + miner.workerName, 'lastShare', dateNowSeconds]);
        redisCommands.push(['expire', config.coin + ':workers:' + miner.login + '+' + miner.workerName, (86400 * 7)]);
    }
