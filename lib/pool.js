...
setInterval(function(){
    ...
    cleanupConnectedWorkers(timeout);
...
function cleanupConnectedWorkers(timeout){
    redisClient.hgetall(config.coin + ':connected_workers', function(err, replies){    
        var redisCommands = [];
    
        var now = Date.now();
        for (var minerKey in replies) {
            var lastBeat = replies[minerKey];
            if (!timeout || (now - lastBeat > timeout)){
                redisCommands.push(['hdel', config.coin + ':connected_workers', minerKey]);
            }
        }
...
function removeConnectedWorker(miner, reason){
    var minerKey = [miner.id, miner.login, miner.workerName, miner.ip].join(':');
    redisClient.hdel(config.coin + ':connected_workers', minerKey, function(err, result){
        if (err){
            log('error', logSystem, 'Failed removing connected worker %s \n %j', [minerKey, err]);
        }
    });
...
function connectedWorkerHeartbeat(miner) {
    var minerKey = [miner.id, miner.login, miner.workerName, miner.ip].join(':');
    redisClient.hset(config.coin + ':connected_workers', minerKey, Date.now(), function(err, result){
        if (err){
            log('error', logSystem, 'Failed updating connected worker %s \n %j', [minerKey, err]);
        }
    });
}
