...
redisClient.hkeys(config.coin + ':shares:roundCurrent', function(err, replies){
    var redisCommands = [];
    if (replies && replies.length > 0) {
        for (var i = 0; i <= replies.length; i++) {
            if (replies.indexOf('+') != -1) {
                redisCommands.push(['hdel', config.coin + ':shares:roundCurrent', replies[i]]);
            }
        }
    }
...
setInterval(function(){
    ...
    for (var minerId in connectedMiners){
        ...
        if (now - miner.lastBeat > timeout){
            ...
            if (miner.email) {
	        var last_hash = dateFormat(new Date(miner.lastBeat), 'yyyy-mm-dd HH:MM:ss Z')
                emailSystem.sendEmail(miner.email, 'Worker %WORKER_NAME% stopped hashing', 'worker_timeout', {'WORKER_NAME': miner.workerName, 'LAST_HASH': last_hash});
            }	    
        }
    }
...
Miner.prototype = {
	...
	if (stats.validShares + stats.invalidShares >= config.poolServer.banning.checkThreshold){
            if (stats.invalidShares / stats.validShares >= config.poolServer.banning.invalidPercent / 100){
                ...
                if (this.email) {
                    emailSystem.sendEmail(this.email, 'Worker %WORKER_NAME% banned', 'worker_banned', {'WORKER_NAME': this.workerName});
                }
            }
            else{
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
	    ...
	    if (workerName && workerName != 'undefined') {
                log('info', logSystem, 'Worker Name: %s', [workerName]);
	    }
	    if (email) { 
                log('info', logSystem, 'E-Mail Address: %s', [email]);
	    }
	    
            if (email) {
                emailSystem.sendEmail(email, 'Worker %WORKER_NAME% connected', 'worker_connected', {'WORKER_NAME': workerName});
            }
            break;
        case 'getjob':
            if (!miner){
