...
function telegramInterval(){
    log('info', logSystem, 'telegram message relay');
    ...
    redisClient.multi([
        ...
        function (error, replies) {
            if (error) {
                ...
                log('info', logSystem, 'Telegram message relay: found %d messages to relay', [replies[0].length]);
...
function emailInterval() {
    log('info', logSystem, 'email message relay');
    ...
    redisClient.multi([
        ...
        function (error, replies) {
            if (error) {
                ...
                log('info', logSystem, 'E-mail message relay: found %d messages to relay', [replies[0].length]);
                for (let t of replies[0]) {
                    let email;
                    try {
                        email = JSON.parse(t);
