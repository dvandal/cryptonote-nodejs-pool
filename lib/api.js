                ...
                redisClient.multi(redisCommands).exec(function(error, replies){
                    for (var i in workersData){
                        ...
                        if (replies[hi]) {
                            ...
                            var sets = JSON.parse(replies[hi]); // [time, avgHashrate, updateCount]
                            for (let j in sets) {
                                var hr = sets[j][1];
                            ...
                            workersData[i]['hashrate_1h']  = sums[0] * 1.0 / (counts[0] || 1);
                            workersData[i]['hashrate_6h']  = sums[1] * 1.0 / (counts[1] || 1);
                            workersData[i]['hashrate_24h'] = sums[2] * 1.0 / (counts[2] || 1);
                        }
                    }
