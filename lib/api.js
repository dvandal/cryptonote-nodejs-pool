...
function broadcastLiveStats(){
    ...
    redisClient.multi(redisCommands).exec(function(error, replies){
        ...
        for (var i = 0; i < addresses.length; i++){
            ...
            stats.lastShare = replies[offset]['lastShare'] ? parseInt(replies[offset]['lastShare']) : 0;
            stats.hashes = replies[offset]['hashes'] ? parseInt(replies[offset]['hashes']) : 0;
            ...
            charts.getUserChartsData(address, paymentsData, function(error, chartsData) {
                ...
                redisClient.multi(redisCommands).exec(function(error, replies){
                    for (var i in workersData){
                        ...
                        workersData[i].roundHashes = replies[i]['roundHashes'] ? parseInt(replies[i]['roundHashes']) : 0;
                    }
...
function handleMinerStats(urlParts, response){
    ...
    if (longpoll){
        ...
        redisClient.multi([
            ...
            var stats = {};
            stats.hashrate = minerStats[address] && minerStats[address]['hashrate'] ? minerStats[address]['hashrate'] : 0;
            stats.lastShare = replies[0]['lastShare'] ? parseInt(replies[0]['lastShare']) : 0;
            stats.hashes = replies[0]['hashes'] ? parseInt(replies[0]['hashes']) : 0;
            stats.roundHashes = minerStats[address] && minerStats[address]['roundHashes'] ? minerStats[address]['roundHashes'] : 0;
