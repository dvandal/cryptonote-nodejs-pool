...
function sendLiveStats(data, response, gzip){
...
function broadcastLiveStats(){
    ...
    for (var uid in liveConnections){
        ...
        if (liveAddressLookup[uid]){
            var address = liveAddressLookup[uid];
            if (minerStats[address]) {        
                data.miner = minerStats[address];
            }
        }        
	sendLiveStats(data, liveConnections[uid], true);
    }    
    ...
    for (var address in addressConnections){
        ...
        redisCommands.push(['keys', config.coin + ':charts:hashrate:' + address + '*']);
    }
    redisClient.multi(redisCommands).exec(function(error, replies){
        ...
        for (var i = 0; i < addresses.length; i++){
            ...
            for (var i=0; i<replies[offset + 2].length; i++) {
                ...
                var miner = keyParts[3];
                var addrParts = getAddressParts(miner);
                if (addrParts.workerName) {
                    if (!minerStats[miner]) continue;
                    var workerData = {
                        name: addrParts.workerName,
                        hashrate: minerStats[miner] && minerStats[miner]['hashrate'] ? minerStats[miner]['hashrate'] : 0,
                        roundHashes: minerStats[miner] && minerStats[miner]['roundHashes'] ? minerStats[miner]['roundHashes'] : 0
                    };
            ...
            charts.getUserChartsData(address, paymentsData, function(error, chartsData) {
                ...
                for (var i in workersData){
                    redisCommands.push(['hgetall', config.coin + ':workers:' + address + '+' + workersData[i].name]);
                }
                redisClient.multi(redisCommands).exec(function(error, replies){
                    for (var i in workersData){
                        if (!replies[i]) continue;
                        workersData[i].lastShare = replies[i]['lastShare'] ? replies[i]['lastShare'] : 0;
                        workersData[i].hashes = replies[i]['hashes'] ? replies[i]['hashes'] : 0;
                    }
                    ...
                    sendLiveStats(data, addressConnections[address], true);
                });
            });
        }
    });
...
function handleMinerStats(urlParts, response){
    ...
    response.writeHead(200, {
        ...
        'Cache-Control': (longpoll) ? 'no-cache' : 'max-age=3',
        'Content-Type': 'application/json',
        'Content-Encoding': longpoll ? 'deflate' : '',
...
function handleServerRequest(request, response) {
    ...
    switch(urlParts.pathname){
        ...
        case '/live_stats':
            ...
            var address = urlParts.query.address;
            if (address) {
                liveAddressLookup[uid] = address;
            }
        
            response.on("finish", function() {
                delete liveConnections[uid];
                delete liveAddressLookup[uid];
            });
            break;
        case '/stats_address':
