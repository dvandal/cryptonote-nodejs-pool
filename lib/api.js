...
function collectStats(){
    ...
    var redisCommands = [
        ...
        // (jagerman: this comment is here to induce a git conflict if something gets added here in
        // a future upstream release -- the 'replies[11]' below would need to be fixed).
    ];

    var haveDonations = false;
    if (config.blockUnlocker.donations && config.blockUnlocker.donations.enabled && config.blockUnlocker.donations.address) {
        haveDonations = true;
        redisCommands.push(['hmget', config.coin + ':workers:' + config.blockUnlocker.donations.address, 'balance', 'paid']);
    }
    ...
    async.parallel({
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                var data = {
                    ...
                    totalDonations: haveDonations ? parseInt(replies[11][0]) + parseInt(replies[11][1]) : 0,
        ...
        config: function(callback){
            callback(null, {
                ...
                donationsEnabled: config.blockUnlocker.donations && config.blockUnlocker.donations.enabled,
                customDonationsEnabled: config.poolServer.customDonationAmount && config.poolServer.customDonationAmount.enabled,
                coin: config.coin,
 ...
 **/
function handleTopMiners(response, comparator) {
    if (!comparator) comparator = compareTopMiners;
    ...
    async.waterfall([
        ...
        function(workerKeys, callback) {
            var redisCommands = workerKeys.map(function(k) {
                return ['hmget', k, 'lastShare', 'hashes', 'donation_level', 'donations'];
            });
...
function compareTopDonators(a,b) {
...
function handleTopDonators(response) {
    return handleTopMiners(response, compareTopDonators);
}
