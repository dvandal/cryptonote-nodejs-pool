...
function extractAverageHashrates(chartdata) {
    ...
    let sets = JSON.parse(chartdata); // [time, avgValue, updateCount]
    for (let j in sets) {
        let hr = sets[j][1];
        ...
        function(workerKeys, callback) {
            ...
            redisClient.multi(redisCommands).exec(function(error, redisData) {
                ...
                for (let i in redisData) {
                    let keyParts = workerKeys[i].split(':');
                    let address = keyParts[keyParts.length-1];
...
function initMonitoring() {
    ...
    for(let module in config.monitoring) {
        let settings = config.monitoring[module];
