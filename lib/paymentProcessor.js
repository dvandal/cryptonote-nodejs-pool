    ...
    async.waterfall([
        ...
        function(balances, minPayoutLevel, callback){
            ...
            async.filter(transferCommands, function(transferCmd, cback){
                ...
                apiInterfaces.rpcWallet(rpcCommand, rpcRequest, transferCmd.rpc, function(error, result){
                    if (error){
                    ...
                    var txHash = config.wallet.bytecoin ? result.transactionHash : result.tx_hash;
                    ...
                    for (var i = 0; i < transferCmd.rpc.destinations.length; i++){
                        ...
                        notify_miners_on_success.push({
                            'AMOUNT': utils.getReadableCoins(destination.amount),
